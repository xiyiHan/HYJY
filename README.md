...
├── mmtools/
│   ├── config.py              配置加载、分层合并、密钥解析（环境变量/密钥文件/内联）
│   ├── transcribers.py        可插拔转写后端（FunASR / faster-whisper）
│   ├── summarizer.py          可配置云端大模型客户端
│   ├── prompts.py             纪要提示词模板
│   ├── glossary.py            术语规范化（同音纠错、写法统一）
│   ├── exporter.py            Word 导出（python-docx，精确控制中文字体）
│   ├── recorder.py            WASAPI loopback + 麦克风录音
│   └── pipeline.py            端到端编排
├── workspace/                 产物：录音、文字稿、纪要
├── models/                    模型缓存（D 盘）
├── .cache/                    运行缓存（D 盘）
└── tests/
    ├── test_offline.py        离线自测 26 项（不加载模型、不联网）
    ├── test_llm_mock.py       云端链路集成测试 12 项（本地模拟服务）
    └── test_recording.py      录音链路诊断（验证麦克风与系统音频）
```
