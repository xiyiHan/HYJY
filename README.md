...
| 页面 | 用途 |
| --- | --- |
| 任务 | 拖入录音文件、排队、开始处理、查看进度与日志 |
| 结果 | 查看生成的文件，一键打开 Word 纪要或所在文件夹 |
| 设置 | 配置云端平台与密钥、转写引擎、热词、模板路径与项目信息 |
| 帮助 | 环境自检（含云端连通性测试）与常见问题 |
...
├── app/                       图形界面
│   ├── main.py                入口
│   ├── main_window.py         主窗口（左导航 + 四页）
│   ├── worker.py              后台线程与进度回报
│   ├── theme.py               样式表
│   └── pages/                 任务 / 结果 / 设置 / 帮助 / 录音对话框
├── mm.py                      CLI 入口
